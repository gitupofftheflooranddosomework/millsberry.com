...
function addShortcut (to_url, default_title) {
	...
	var user_title = prompt ("What name do you want to give this shortcut?\nLeave this blank to use the page's existing title.","");
	if (user_title.length > 0) {
		assign_title = base64_encode (utf8_encode (user_title));
	}
	window.location='/process_shortcut.phtml?action=add&url=' + to_url + '&name=' + assign_title;
}
...
function flash_object (src, width, height, flash_vars, version, scale) {
	...
	document.write ("<embed type='application/x-shockwave-flash' src='" + src + "' width='" + width + "' height='" + height + "' quality='high' wmode='transparent' scale='" + scale + "' salign='lt' pluginspage='http://www.macromedia.com/shockwave/download/index.cgi?P1_Prod_Version=ShockwaveFlash' AllowScriptAccess='always' FlashVars='" + flash_vars + "'></embed>");
